...
(function (global) {
  ...
  var R = 0.034;            // 球半径（m）——按真实比例（真实约 2.25 吋=2.86cm 半径），桌宽比例接近 9 尺真实桌
  var D = R * 2;            // 球直径
  var TABLE_W = 2.4;        // 内沿宽（9 尺桌 2.54m 的近似比例）
  var TABLE_H = 1.2;        // 内沿高（2:1）

  var CORNER = 0.060;       // 角袋捕获半径（≈1.8 球半径，符合真实袋口≈2 球宽的观感）
  var SIDE = 0.070;         // 中袋捕获半径（开口略大）
