/* 海战棋联机层：封装 Cloudflare Worker 通信（建房间 / 加入 / WebSocket 同步）。
 *
 * 与 QuoridorOnline 共用同一个 Worker（quoridor-mp），通过 ?game=bs 区分游戏类型，
 * 服务端按 gameType 选引擎并下发「红化视图」（绝不泄露对手船位）。
 *
 * 消息协议（海战棋）：
 *   hello {player}        → welcome {player} + 首份 state 视图
 *   place {player, layout} → 上报布阵；双方就位后服务端开火阶段并广播
 *   fire  {player, r, c}  → 开火；服务端权威判定后广播红化视图
 *   reset {player}        → 新对局（需双方确认：req_new / res_new 中继）
 * 服务端还下发 players（双方在线状态）、status、error、close。
 */
(function () {
  'use strict';
  ...
  Online.prototype.sendPlace = function (layout) {
    this._wsSend({ type: 'place', player: this.player, layout: layout });
  };
  // 开火
  Online.prototype.sendFire = function (r, c) {
    this._wsSend({ type: 'fire', player: this.player, r: r, c: c });
  };
  // 新对局：请求 / 确认（双向确认，双方点头才重置）
  ...
  window.BattleshipOnline = Online;
})();
