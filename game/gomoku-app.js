...
(function () {
  ...
  function renderTimerUI() {
    ...
    if (el.clockName0) el.clockName0.textContent = onlineMode ? (myPlayer === 0 ? myName() : '对手') : names[0];
    if (el.clockName1) el.clockName1.textContent = onlineMode ? (myPlayer === 1 ? myName() : '对手') : names[1];
  }
...
var confirmMode = false;          // 触屏确认模式（手机/平板）
  ...
  function boot() {
    ...
    if (m === 'ai') { mode = 'ai'; onlineMode = false; humanColor = 1; aiSide = 2; newGame(true); }
      ...
      if (window.BotDriver) BotDriver.attach(online, { game: 'wm' });
