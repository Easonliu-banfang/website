/* 八球台球联机客户端 —— 与四子棋/五子棋同构：等待室(ready/start/notify/leave) 后跳转 3D 对局
 * 实际对局走 Nchan 哑中继（@tailuge/messaging），本客户端只负责 /api/room 状态机等待室。
 */
(function (global) {
  'use strict';
  ...
  Online.prototype.createRoom = function () {
    var self = this;
    return fetchJSON(WS_BASE + '/api/room?game=' + GAME, { method: 'POST' })
      .then(function (d) { self.code = d.code; return d.code; });
  };

  Online.prototype.joinRoom = function (code) {
    this.code = code;
    return fetchJSON(WS_BASE + '/api/room/' + code + '?game=' + GAME).then(function (d) {
  ...
  Online.prototype.send = function (obj) {
