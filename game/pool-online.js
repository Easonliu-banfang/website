/* 八球台球联机层：封装 Cloudflare Worker 通信（建房间 / 加入 / WebSocket 同步）。
 *
 * 与服务端共用同一 Worker（quoridor-mp），?game=pl 区分游戏类型。
 * 台球是「开放信息 + 连续物理」：
 *   1) 服务端权威推进物理并实时下发 live 快照（对方无延迟观战）
 *   2) 击球后服务端结算规则并广播权威 state（含 world/match）
 *   3) shoot / place_cue / choose 发到服务端，服务端校验回合后执行
 */
(function () {
  'use strict';
  ...
  Online.prototype._wsSend = function (obj) {
    if (this.ws && this.ws.readyState === 1) { try { this.ws.send(JSON.stringify(obj)); } catch (e) {} return; }
  ...
  Online.prototype.createRoom = function () {
    var self = this;
    return fetchJSON(WORKER_BASE + '/api/room?game=' + GAME, { method: 'POST' })
  ...
  Online.prototype.joinRoom = function (code) {
    this.code = code;
    var self = this;
    return fetchJSON(WORKER_BASE + '/api/room/' + code + '?game=' + GAME)
      .then(function (d) {
  ...
  Online.prototype._open = function () {
    ...
    return new Promise(function (resolve, reject) {
      ...
      ws.onmessage = function (e) {
        ...
        else if (m.type === 'live') self._emit('live', m);
  ...
  Online.prototype.sendPlaceCue = function (x, y) { this._wsSend({ type: 'place_cue', player: this.player, x: x, y: y }); };
  Online.prototype.sendShoot = function (shot) {
    this._wsSend({ type: 'shoot', player: this.player, aimX: shot.aimX, aimY: shot.aimY, power: shot.power, top: shot.top, side: shot.side, callPocket: shot.callPocket });
  };
  Online.prototype.sendChoose = function (group) { this._wsSend({ type: 'choose', player: this.player, group: group }); };
