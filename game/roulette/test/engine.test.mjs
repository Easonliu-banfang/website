 ...
 */
import {
  createGame, load, shoot, useItem, peek, view, aiDecide,
  MAX_LIVES, MAX_ROUND, ROUND_LIVES, ROUND_ITEMS,
...
console.log('--- 开局状态 ---');
{
  ...
  eq('第 1 轮双方各 2 命', g.lives.me, ROUND_LIVES[1]);
  eq('恶魔 2 命', g.lives.foe, ROUND_LIVES[1]);
  eq('第 1 轮无道具', g.items.me.length, 0);
  eq('恶魔也无道具', g.items.foe.length, 0);
  eq('轮次=1', g.round, 1);
  eq('玩家先手', g.turn, 'me');
...
console.log('--- 每回合限 1 道具 ---');
{
  ...
  g.items.me = ['magnifier', 'beer'];        // round1 无道具，手动给
...
console.log('--- 射击判定：射自己 ---');
{
  ...
  eq('自射空弹不扣命', g.lives.me, ROUND_LIVES[1]);
...
console.log('--- 射击判定：射恶魔 ---');
{
  ...
  shoot(g, 'me', 'foe');
  eq('射恶魔空弹不扣命', g.lives.foe, ROUND_LIVES[1]);
...
console.log('--- 弹仓打空重装补道具 ---');
{
  ...
  g.round = 2;                           // round2 每次装弹发 2 道具
  g.shell = [false]; g.idx = 0;
  g.items.me = [];
  const before = g.items.me.length;
  shoot(g, 'me', 'self');            // 空弹自射 + 弹仓打空 → 重装 + 补道具
  ok('弹仓已重装', g.shell.length >= 2);
  ok('round2 补发 2 道具', g.items.me.length - before >= ROUND_ITEMS[2]);
}

console.log('--- 3 轮制：round1 命尽推进 / round3 突死结束 ---');
{
  const g = createGame(31);
  g.shell = [true, true]; g.idx = 0;
  g.lives.foe = 1;
  const r = shoot(g, 'me', 'foe');   // round1 恶魔命尽
  ok('round1 恶魔命尽不结束', r.over !== true);
  eq('进入 round2', g.round, 2);
}
{
  const g = createGame(32);
  g.round = 3; g.lives = { me: 5, foe: 1 };
  g.shell = [true, true]; g.idx = 0;
  const r = shoot(g, 'me', 'foe');   // round3 恶魔命尽 → 整场胜利
  ok('round3 恶魔命尽 → 整场胜利', r.over === true && r.winner === 'me');
  ok('游戏结束', g.over === true);
}
...
console.log('--- 视图 ---');
{
  ...
  ok('view 含 maxLives(round1=2)', v.maxLives === ROUND_LIVES[1]);
  ok('view 含 round', v.round === 1 && v.maxRound === 3);
}
