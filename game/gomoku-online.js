/* 五子棋联机层：封装 Cloudflare Worker 通信（建房间 / 加入 / WebSocket 同步）。
 * 与 BattleshipOnline 共用同一 Worker（quoridor-mp），通过 ?game=wm 区分，服务端权威校验落子与胜负。
 * 消息协议（五子棋）：
 *   hello {player}          → welcome {player} + 首份 state
 *   move  {player, r, c}    → 落子；服务端校验后广播 state
 *   reset {player}          → 新对局（需双方确认：req_new / res_new 中继）
 * 服务端还下发 players / status / error / close / ping / pong。
 */
(function () {
  'use strict';
